...
class DBIntegrity(DBTester):
    def test_file_pragma(self):
        with self.conn as c:
            filetags._column_safety(c, 'files', cols='*')
            filetags._column_safety(c, 'files', cols=('name', 'size', 'is_dir'))
            filetags._column_safety(c, 'files', cols=('directory', 'mod_time', 'is_dir', 'is_dir', 'size'))
            with self.assertRaises(OperationalError):
                filetags._column_safety(c, 'files', cols=('name', 'directory', 'isdir', 'size'))
            with self.assertRaises(OperationalError):
                filetags._column_safety(c, 'files', cols=('dog', 'name', 'is_dir'))

    def test_tag_pragma(self):
        with self.conn as c:
            filetags._column_safety(c, 'tags', cols='*')
            filetags._column_safety(c, 'tags', cols=('key', 'value'))
            filetags._column_safety(c, 'tags', cols=('value', 'id', 'id'))
            with self.assertRaises(OperationalError):
                filetags._column_safety(c, 'tags', cols=('valkey',))
            with self.assertRaises(OperationalError):
                filetags._column_safety(c, 'tags', cols=('value', 'id', 'ke'))
        ...
        for fp in self.fake_filepaths:
            ...
            with self.conn as c:
                ...
                for pair in self.pairs:  # For each tag, we go through.
                    ...
                    filetags._relate_tag_and_file(c, d, n, *pair)
                    ...
                    filetags._unrelate_tag_and_file(c, d, n, *pair)
                    res = c.execute("""SELECT * FROM filetag_junction WHERE
                            file_id = ? AND tag_id = ?""", (file_id, tag_id)).fetchall()
            ...
            for fp in self.fake_filepaths:
                ...
                for pair in self.pairs:
                    with self.subTest(fp=fp, pair=pair):
                        ...
                        filetags._relate_tag_and_file(c, d, n, *pair)  # Relating...
                        # Here we get all of the tags in the standard format.
                        f_tags = filetags.tags_of_file(c, d, n, cols=('key', 'value'))
                        logging.debug("Tags we got back: {}.".format(', '.join('='.join(_) for _ in f_tags)))
                        self.assertEqual(seen, f_tags)
        ...
        with self.conn as c:
            ...
            for fp in self.fake_filepaths:
                ...
                with self.subTest(fp=fp):
                    ...
                    filetags._relate_tag_and_file(c, d, n, *tag)
                    tagfiles = filetags.files_of_tag(c, *tag, cols=('directory', 'name'))
